...
import type { SummarizerConfig, SummaryResult } from './types';
...
function parseSummaryOutput(raw: string): {
  ...
  if (jsonMatch) {
    try {
      ...
      // Summary is everything before the JSON block
  ...
  // Fallback: if no topics extracted, try simple tag detection
  if (topics.length === 0) {
...
export async function summarize(
  ...
  const systemPrompt = `你是桌面宠物"利落"。请用3-5句话总结以下文件的核心内容。
要求：
- 先给结论，再给细节
- 如果内容质量差，直说
- 语气简洁直接，不用客套话
- 最后输出JSON格式：{"topics": ["主题1", "主题2"], "related_file": "关联的旧文件名或null"}`;
  ...
  // Build context from recent files
  const recentContext =
  ...
  if (tokenCount <= limits.maxDirectTokens) {
    ...
    // Extract topics separately for structured output
